...
export const TELOP_PRESETS: TelopPreset[] = [
  ...
  },
  {
    id: "endroll",
    name: "エンドロール（流れて停止）",
    description: "下から上へ流れ、好きな位置で止まってフェードアウト。",
    ...
    style: s({
      ...
      background: { enabled: false, color: "#000", opacity: 0.5, paddingX: 20, paddingY: 10, radius: 8 },
      animation: { in: "scrollUp", out: "fade", inDuration: 10, outDuration: 1.5, holdDuration: 5 },
    }),
  },
  {
    id: "marquee",
    name: "横スクロール（流れ続ける）",
    description: "右から左へ流れるニューステロップ風。クリップ長で終了。",
    sampleText: "※ イヤホン推奨　本編は 0:15 から　#歌ってみた",
    x: 0.5,
    y: 0.12,
    duration: 8,
    style: s({
      ...
      fontWeight: 600,
      color: "#ffffff",
      stroke: { enabled: true, color: "#1e3a5f", width: 4 },
      shadow: { enabled: false, color: "#000", blur: 0, offsetX: 0, offsetY: 0 },
      background: { enabled: true, color: "#5b8def", opacity: 0.85, paddingX: 24, paddingY: 8, radius: 4 },
      animation: { in: "scrollLeft", out: "fade", inDuration: 0, outDuration: 0.6, holdDuration: 0 },
    }),
  },
  {
    id: "credit",
